 ...
 *
 * noise_buf  : 1.7 kHz sine wave  (noise-reference mic)
 * voice_buf  : 1 kHz sine + 1.7 kHz sine wave  (voice mic)
 * n_samples  : number of samples to generate (must be HOP_SIZE)
 *
 * Both channels share the same noise phase so the noise
...
/* Reset signal generator phase for deterministic output across captures. */
